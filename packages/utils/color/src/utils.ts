...
export const RGBStringToRGB = (
  ...
  const result = /^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/.exec(string);
  return result
    ? {
...
export const RGBToRGBString = (r: number, g: number, b: number): string =>
  `rgb(${r}, ${g}, ${b})`;

export const RGBAStringToRGBA = (
  string: string
